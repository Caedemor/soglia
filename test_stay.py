...
def test_held_recognizer():
    ...
    not_held = ["Driver 1", "Driver 2", "SGL", "DBL", "No. of rooms / people",
...
def test_park_reconciliation():
    ...
    assert rec == {"expected": 41, "named": 23, "pending": 18, "overage": 0}, rec
...
def test_reconcile_units():
    ...
    assert rec == {"expected": 21, "named": 4, "pending": 18, "overage": 1}, rec
    assert completeness_status(rec) == "awaiting_completion"
...
def test_other_lists_complete():
    ...
    assert len(p.guests) == 55 and not [s for s in p.stays
